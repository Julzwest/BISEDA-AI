...
export default function DateRehearsal() {
  ...
  const scenarios = [
    ...
    },
    {
      ...
      roleType: 'family' // AI plays as family member or friend
    },
    {
...
`Open the door warmly and greet ${safeUserName}. Maybe offer them a drink or compliment them. Be welcoming but show you're a protective parent sizing them up.` :
selectedScenario.roleType === 'stranger' ?
`Notice ${safeUserName} approaching. React based on your personality - smile if friendly, look intrigued if confident, be shy if reserved.` :
...
${safeDateName} (speaking naturally, 1-3 sentences):`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Build conversation history as text for prompt
      const conversationHistory = updatedMessages.map(m => 
        `${m.sender === 'user' ? userName || 'User' : dateName}: ${m.text}`
      ).join('\n');
